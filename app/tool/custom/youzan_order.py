...
class YouzanOrderTool(BaseTool):
    ...
    async def execute(self, order_no: str) -> ToolResult:
        ...
        try:
            ...
            # 记录请求参数
            logger.info(f"查询订单：{order_no}, 请求参数: {json.dumps(payload)}")
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    logger.info(f"HTTP响应预览: {preview}")
                    ...
                    if response.status != 200:
                        ...
                        return ToolResult(error=f"查询失败，状态码: {response.status}")

                    try:
                        data = json.loads(raw_response)
                        # 只记录数据类型，避免日志过大
                        logger.info(f"解析后的响应数据类型: {type(data)}")
                    except json.JSONDecodeError as e:
                        logger.error(
                            f"JSON解析错误: {str(e)}, 原始内容: {raw_response}"
                        )
                        return ToolResult(error=f"响应数据不是有效的JSON格式: {str(e)}")

                    # 处理响应
                    return self._process_response(order_no, data)
        except Exception as e:
            logger.error(f"订单查询错误: {str(e)}", exc_info=True)
            return ToolResult(error=f"订单查询出错: {str(e)}")
    ...
    def _process_response(self, order_no: str, data: Dict) -> ToolResult:
        ...
        try:
            ...
            # 检查是否有错误响应
            logger.info(f"开始处理响应数据: 类型={type(data)}")
            ...
            # 记录第一层键
            logger.info(f"响应数据顶层键列表: {list(response_data.keys())}")

            # 首先检查API是否返回成功
            is_success = False
            if "success" in response_data:
                logger.info(f"API响应success字段: {response_data['success']}")
                if response_data["success"] is True:
                    is_success = True

            if "code" in response_data:
                logger.info(f"API响应code字段: {response_data['code']}")
                if response_data["code"] == 200:
                    is_success = True

            if "statusCode" in response_data:
                logger.info(f"API响应statusCode字段: {response_data['statusCode']}")
            ...
            if not is_success:
                ...
                if "message" in response_data and response_data["message"]:
                    logger.info(f"API返回成功消息: {response_data['message']}")
            ...
            logger.info(f"订单数据字段列表: {list(order.keys())}")
            ...
            if "mainOrderInfo" in order and isinstance(order["mainOrderInfo"], dict):
                ...
                logger.info("从mainOrderInfo字段提取订单主信息")
            else:
                main_order_info = order
                logger.info("使用当前级别数据作为订单主信息")

            # 记录主订单信息字段
            if isinstance(main_order_info, dict):
                logger.info(f"主订单信息字段: {list(main_order_info.keys())}")
            ...
            if "orderItems" in order and order["orderItems"]:
                ...
                logger.info("从orderItems字段提取商品信息")
            elif "items" in order and order["items"]:
                items = order["items"]
                logger.info("从items字段提取商品信息")
                ...
                for field in ["orderItems", "items", "list"]:
                    if field in order["itemInfo"] and order["itemInfo"][field]:
                        ...
                        logger.info(f"从itemInfo.{field}字段提取商品信息")
                        break
            ...
            if items:
                ...
                for idx, item in enumerate(items, 1):
                    ...
                    if not title or title == "N/A":
                        title = item.get("itemName", "N/A")
                    order_info.append(f"  {idx}. {title}")

                    # 尝试不同的价格字段名称
                    price = 0
                    for price_field in ["price", "unitPrice", "itemPrice"]:
                    ...
                    # 尝试不同的数量字段名称
                    num = 0
                    for num_field in ["num", "quantity", "itemNum"]:
                    ...
                    # 计算总价或获取总价字段
                    total_price = 0
                    for total_field in ["totalPrice", "totalFee", "payment"]:
            ...
            if "buyerInfo" in order and order["buyerInfo"]:
                ...
                logger.info("从buyerInfo字段提取买家信息")
            elif "buyer" in order and order["buyer"]:
                buyer = order["buyer"]
                logger.info("从buyer字段提取买家信息")

            if buyer:
                logger.info(f"买家信息字段: {list(buyer.keys())}")
                order_info.append("\n买家信息:")
                buyer_id = (
                    buyer.get("buyerId")
                    or buyer.get("yzUid")
                ...
                if "mobile" in buyer and buyer["mobile"]:
                    order_info.append(f"  联系电话: {buyer['mobile']}")
    ...
    def _find_main_orders(self, data, order_no, path=""):
        ...
        # 开始检查前记录当前要搜索的路径
        if not path:
            logger.info(f"开始在数据中查找订单{order_no}")

        # 直接路径
        if "mainOrders" in data:
            logger.info(f"在路径 {current_path}.mainOrders 找到订单数据")
            return data["mainOrders"]

        # 检查data字段
        if "data" in data:
            logger.info(f"检查 {current_path}.data 路径")
            if isinstance(data["data"], dict):
                # data字段下的mainOrders
                if "mainOrders" in data["data"]:
                    logger.info(f"在路径 {current_path}.data.mainOrders 找到订单数据")
                    return data["data"]["mainOrders"]

                # data下可能有order或orders字段
                if "order" in data["data"]:
                    logger.info(f"在路径 {current_path}.data.order 找到订单数据")
                    return data["data"]["order"]

                if "orders" in data["data"]:
                    if isinstance(data["data"]["orders"], list):
                        logger.info(f"在路径 {current_path}.data.orders 找到订单列表")
                        # 可能需要匹配订单号
                ...
                for key, value in data["data"].items():
                    if isinstance(value, (dict, list)):
                        logger.info(f"递归检查 {current_path}.data.{key}")
            ...
            # data字段可能直接是列表
                ...
                logger.info(f"在路径 {current_path}.data 发现列表数据")
                # 检查第一个元素
                return data["data"]
        ...
        for top_field in ["result", "response", "orderData", "content"]:
            if top_field in data and data[top_field]:
                logger.info(f"检查 {current_path}.{top_field} 路径")
                if isinstance(data[top_field], dict):
                    ...
                    # 如果是列表且有内容，返回该列表
                    logger.info(f"在路径 {current_path}.{top_field} 找到列表数据")
        ...
        for key in ["order", "orders", "list", "items"]:
            if key in data:
                if isinstance(data[key], dict):
                    logger.info(f"在路径 {current_path}.{key} 找到订单数据(字典)")
                    return data[key]
                elif isinstance(data[key], list) and data[key]:
                    logger.info(f"在路径 {current_path}.{key} 找到订单列表")
                    return data[key]
        ...
        if not path:  # 只在顶层记录，避免递归时重复记录
            try:
                ...
                if "data" in data and data["data"]:
                    if isinstance(data["data"], dict):
                        logger.info(f"data字段内的键: {list(data['data'].keys())}")
                    elif isinstance(data["data"], list):
                        logger.info(f"data字段是列表，长度: {len(data['data'])}")
                        if data["data"]:
